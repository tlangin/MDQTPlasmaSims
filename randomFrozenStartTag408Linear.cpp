...
/*                                                                                                                                        */
/*    g++ -std=c++11 -fopenmp -o tag408 -O3 simOptPumping408.cpp -lm -I/home/tkl1/usr/include -L/users/tkl1/user/lib64                    */
/*                                                                                                                                        */
...
char saveDirectory[256] = "data408/";//main directory.  A subfolder titled  pumpTime___PumpStart___Om___Det___Density__Ge__Number___ will be created.  Subsubfolders for each job will be created.  data will be stored in the job folders;

//pumping parameters
double detuning=-2.5;//pump detuning in units \gamma_{408}
double Om=0.7;//rabi frequency in units \gamma_{408}
double tpumpreal=0.0000002;// in seconds
...
{  
  ...
  for(i=0;i<N;i++){
	...
	cx_mat hamCouplingTermSP = -Om/2*wvFn2*wvFn4.t()*sqrt(gs[3])-Om/2*wvFn2*wvFn6.t()*sqrt(gs[5])-Om/2*wvFn1*wvFn3.t()*sqrt(gs[0])-Om/2*wvFn1*wvFn5.t()*sqrt(gs[2]);
