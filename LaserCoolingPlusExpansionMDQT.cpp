...
#define tmax 5                // maximum simulation time: on 07/25/18 tested for 3000 particles at n=2: a sim for tmax = 2.25 concluded in 1 hr 45 mins, so to keep your sims under 8 hours simulate no more than t= 9.5 at a time...obviously this depends on number of particles and on density (lower density means more quantum steps per MD step means LONGER Time)
...
double density = 150;              //units of 10^14 m^-3
...
#define N0 3000                // average particle number in simulation cell
double detuning=-1;//SP detuning normalized by \gamma_{SP}
double detuningDP=0;//DP detuning normalized by \gamma_{SP} (not a typo: all "quantum times" normalized by \gamma_{SP}, see TKL PhD Thesis
...
{  
  ...
  cx_mat hamCouplingTermSP,hamCouplingTermDP,hamEnergyTermP,hamEnergyTermD,hamEnergyTerm,hamDecayTerm,hamWithoutDecay,hamil;
...
#pragma omp parallel private(i,j,wvFn,velQuant,velPlas,kick,densMatrix,p23,p14,p25,p16,p96,p105,p114,p123,p76,p85,p94,p103,totalDetRightSP,totalDetLeftSP,dp,rand,dtHalf,prefactor,hamCouplingTermSP,hamCouplingTermDP,hamEnergyTermP,hamEnergyTermD,hamEnergyTerm,hamDecayTerm,dpmat,hamWithoutDecay,hamil,matPrefactor,wvFnStepped,k1,k2,k3,k4,wvFnk1,wvFnk2,wvFnk3,dpmatTerms,rand2,rand3,norm3,norm4,norm5,norm6,totalNorm,prob3,prob4,prob5,prob6,randDOrS,randDir,sDecay,popS,popP,popD) shared(V,R,N,wvFns,plasVelToQuantVel,gamToEinsteinFreq,cs,wvFn1,wvFn2,wvFn3,wvFn4,wvFn5,wvFn6,wvFn7,wvFn8,wvFn9,wvFn10,wvFn11,wvFn12,vKick,Om,detuning,detuningDP,OmDP,decayRatioD5Halves,gs,I,kRat,numStates,zero_mat1,dtQuant,expDetuning,tPart)
  {
#pragma omp for 
	...
	kick = 1*vKick*Om*(p23(0,0).imag()*sqrt(gs[0])+p14(0,0).imag()*sqrt(gs[2])-p25(0,0).imag()*sqrt(gs[4])-p16(0,0).imag()*sqrt(gs[5]))*dtQuant*gamToEinsteinFreq+vKickDP*(OmDP/sqrt(decayRatioD5Halves))*(p96(0,0).imag()*sqrt(gs[8])+p105(0,0).imag()*sqrt(gs[11])+p114(0,0).imag()*sqrt(gs[14])+p123(0,0).imag()*sqrt(gs[17])-p76(0,0).imag()*sqrt(gs[6])-p85(0,0).imag()*sqrt(gs[9])-p94(0,0).imag()*sqrt(gs[12])-p103(0,0).imag()*sqrt(gs[15]))*dtQuant*gamToEinsteinFreq;//dhange in velocity due to quantum force, see TKL thesis for full calculation
	...
	hamCouplingTermSP = -Om/2*wvFn2*wvFn3.t()*sqrt(gs[0])-Om/2*wvFn1*wvFn4.t()*sqrt(gs[2])-Om/2*wvFn2*wvFn5.t()*sqrt(gs[4])-Om/2*wvFn1*wvFn6.t()*sqrt(gs[5]);//coupling terms are from lasers and depend on C-G coeffs (hence the sqrt(gs) terms
        hamCouplingTermDP = -OmDP/2*wvFn7*wvFn6.t()*sqrt(gs[6]/decayRatioD5Halves)-OmDP/2*wvFn8*wvFn5.t()*sqrt(gs[9]/decayRatioD5Halves)-OmDP/2*wvFn9*wvFn4.t()*sqrt(gs[12]/decayRatioD5Halves)-OmDP/2*wvFn10*wvFn3.t()*sqrt(gs[15]/decayRatioD5Halves)-OmDP/2*wvFn12*wvFn3.t()*sqrt(gs[17]/decayRatioD5Halves)-OmDP/2*wvFn11*wvFn4.t()*sqrt(gs[14]/decayRatioD5Halves)-OmDP/2*wvFn10*wvFn5.t()*sqrt(gs[11]/decayRatioD5Halves)*exp(I*2.*(expDetuning+velQuant)*(1+kRat)*tPart[i]*gamToEinsteinFreq)-OmDP/2*wvFn9*wvFn6.t()*sqrt(gs[8]/decayRatioD5Halves)*exp(I*2.*(velQuant+expDetuning)*(1+kRat)*tPart[i]*gamToEinsteinFreq);
	...
	hamWithoutDecay=hamEnergyTerm+hamCouplingTermSP+hamCouplingTermSP.t()+hamCouplingTermDP+hamCouplingTermDP.t();//add all the non-decay terms together, including hermitian conjugates of coupling terms!
	...
	dpmat=cx_mat(zero_mat1,zero_mat1);
	for(j=0;j<18;j++){
	  dpmatTerms[j] = dtQuant*gamToEinsteinFreq*wvFnk1.t()*cs[j].t()*cs[j]*wvFnk1;
	  dpmat=dpmat+dpmatTerms[j]*gs[j];
	}
	dp = dpmat(0,0).real();
	prefactor = 1/sqrt(1-dp);
	...
	dpmatTerms[18];
        dpmat=cx_mat(zero_mat1,zero_mat1);
	for(j=0;j<18;j++){
	  dpmatTerms[j] = dtQuant*gamToEinsteinFreq*wvFnk2.t()*cs[j].t()*cs[j]*wvFnk2;
	  dpmat=dpmat+dpmatTerms[j]*gs[j];
	}
        dp = dpmat(0,0).real();
        prefactor = 1/sqrt(1-dp);
	...
	dpmat=cx_mat(zero_mat1,zero_mat1);
	for(j=0;j<18;j++){
	  dpmatTerms[j] = dtQuant*gamToEinsteinFreq*wvFnk3.t()*cs[j].t()*cs[j]*wvFnk3;
	  dpmat=dpmat+dpmatTerms[j]*gs[j];
	}
	dp = dpmat(0,0).real();
	prefactor = 1/sqrt(1-dp);
      ...
      if (rand2<prob3)//if ion was found in state 3 (mJ=+3/2)
	{
	  if(sDecay){//only "S" state that 3 can decay to is 2 (mJ=+1/2) (wvFn(1,0) = state 2...c++ is zero indexed!)
	    ...
	    rand3 = drand48();
	    if(rand3<gs[17]/decayRatioD5Halves){
	      wvFn(11,0).real(1);
	    }
	    else if(rand3<gs[17]/decayRatioD5Halves+gs[16]/decayRatioD5Halves){
	      wvFn(10,0).real(1);
	    }
	    else{
	      wvFn(9,0).real(1);
      ...
      else if(rand2<prob3+prob4)//if ion in state 4
	{
	  if(sDecay){//can decay to 1 or 2, roll for probability
	    ...
	    rand3=drand48();
	    if(rand3<gs[14]/decayRatioD5Halves){
	      wvFn(10,0).real(1);
	    }
	    else if(rand3<gs[14]/decayRatioD5Halves+gs[13]/decayRatioD5Halves){
	      wvFn(9,0).real(1);
	    }
	    else{
	      wvFn(8,0).real(1);
      ...
      else if(rand2<prob3+prob4+prob5)//if in state 5
	{
	  if(sDecay){//can decay to 1 or 2, roll for probability
	    ...
	    rand3=drand48();
	    if(rand3<gs[11]/decayRatioD5Halves){
	      wvFn(9,0).real(1);
	    }
	    else if(rand3<gs[11]/decayRatioD5Halves+gs[10]/decayRatioD5Halves){
	      wvFn(8,0).real(1);
	    }
	    else{
	      wvFn(7,0).real(1);
      ...
      else//if in state 6
	{
	  if(sDecay){//can only decay to 1
	    ...
	    rand3=drand48();
	    if(rand3<gs[8]/decayRatioD5Halves){
	      wvFn(8,0).real(1);
	    }
	    else if(rand3<gs[8]/decayRatioD5Halves+gs[7]/decayRatioD5Halves){
	      wvFn(7,0).real(1);
	    }
	    else{
	      wvFn(6,0).real(1);
...
int main(int argc, char **argv)
{
  ...
  gs[4]=1./3;
  gs[5]=1.;
  gs[6]=decayRatioD5Halves*2./3;
  gs[7]=decayRatioD5Halves*4./15;
  gs[8]=decayRatioD5Halves*1./15;
  gs[9]=decayRatioD5Halves*2./5;
  gs[10]=decayRatioD5Halves*2./5;
  gs[11]=decayRatioD5Halves*1./5;
  gs[12]=decayRatioD5Halves*1./5;
  gs[13]=decayRatioD5Halves*2./5;
  gs[14]=decayRatioD5Halves*2./5;
  gs[15]=decayRatioD5Halves*1./15;
  gs[16]=decayRatioD5Halves*4./15;
  gs[17]=decayRatioD5Halves*2./3;
