...
//comple command: g++ -std=c++11 -o tag422 -O3 MCMDSpinTagQTSimLinear422.cpp -lm -I/home/tkl1/usr/include -L/users/tkl1/user/lib64

/*  We have also added the ability to simulate the optical pumping     */
/*  done by cross cicrularly polarized counter propagating             */
/*  red detuned 422 lasers                                             */
...
char saveDirectory[256] = "data422/";//main directory.  A subfolder titled  Gamma_Kappa_Number___ will be created.  Subsubfolders for each job will be created.  data will be stored in the job folders;
...
//pumping variables (Omega, detuning, tpump)
double tpumpreal=0.00000005;// in seconds
...
double gamToEinsteinFreq = 174.07*.894/sqrt(n);// ratio of gamma=1.26e8 to einstein freq
int plasmaToQuantumTimestepRatio = (int) round(87*.894/sqrt(n));//1/5 of above quantity because MD timestep is 0.002omega-1 while quantum timestep of 0.01gamma-1
double quantumTimestep = timeStep/plasmaToQuantumTimestepRatio;//timestep for quantum evolution (0.01Gam) in plasma time units
double plasVelToQuantVel=1.1821*pow(n,1./6)*.967;//conversion factor for going from plasma to quantum velocities (norm by a\omega and k/\gamma respectively)
double tpump = tpumpreal*813490*sqrt(n);//in omega_{E}t
int pumpMDTimeSteps = (int) round(tpump/timeStep);
double decayRatio=0.0753;//ratio of D decay to S decay (9.5)/(126), see NIST spectral data
...
cx_mat wvFn5=cx_mat(ident.col(4),mat(numStates,1,fill::zeros));//D states
...
{
  //start with particles tagged with prob prop to v.  Writeup full distribution as well
