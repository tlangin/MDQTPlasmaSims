...
/*  done by a single circularly polarized resonant 408 laser.          */
/*  Ideally, this will pump ions out of the spin up state with         */  
/*  probability given by P(v)\propto v^2, allowing for measurement     */
/*  of longitudinal viscosity                                          */
...
/*                                                                                                                                        */
/*    g++ -std=c++11 -fopenmp -o tagQuad -O3 simQuadPumping.cpp -lm -I/home/tkl1/usr/include -L/users/tkl1/user/lib64                     */
/*                                                                                                                                        */
...
char saveDirectory[256] = "data/";//main directory.  A subfolder titled  pumpTime___PumpStart___Om___Det___Density__Ge__Number___ will be created.  Subsubfolders for each job will be created.  data will be stored in the job folders;
...
int plasmaToQuantumTimestepRatio = (int) round(34.81/sqrt(density));//1/5 of above quantity because MD timestep is 0.002omega-1 while quantum timestep of 0.01gamma-1
...
double LongKin;     								     // velocity^2 autocorrelation function
...
void output(void)
{
    ...
    for(i=0;i<N;i++) // add weight functions exploiting isotropy of Pvel (don't do that anymore...separate "x" from y,z).  Also record moments of tagged distro
    {
      currVx = V[0][i];
      if(SpinUpList[i])
	{
...
/***********************************************************************/
/*                                                                     */
/*                Longitudinal stress AUTOCORRELATION                  */
/*                                                                     */
/***********************************************************************/
...
void Zfunc(int c1V)
{
  ...
  double avgVelSq=totalVelSq/N;
    ...
    for(int j = 0; j<N ; j++)                           
	{                                          
	  LongKin += 1/((double)(N)) * (Vholder[j]*Vholder[j]-avgVelSq)*(V[0][j]*V[0][j]-avgVelSq);
	  	// Calculate velocity autocorrelation function at this time
	}				
}

void printLongKin(double time)
{
	char namebuf[256];
	char dataDirCopy[256];
	FILE *fa;
	strcpy(dataDirCopy,saveDirectory);
	strcpy(namebuf,strcat(dataDirCopy,"vSquareAutoCorr.dat"));
	fa=fopen(namebuf,"a");
	fprintf(fa,"%lg\t%lg\n",time,LongKin);
	fclose(fa);
}
